...
def canon(obj: dict) -> bytes:
    """
    Canonicalize a dictionary using JCS (RFC 8785) if available,
    otherwise use sorted JSON.
    """
    if HAS_JCS:
...
def seal(
    ...
    # Generate fresh 32-byte nonce (per recommendation)
...
def seal_with_passphrase(
    ...
    # Derive key with Argon2id (recommended parameters)
    key = hash_secret_raw(
...
def serialize_commitment(commitment: dict) -> bytes:
    """Serialize commitment using JCS (RFC 8785) or sorted JSON."""
    return canon(commitment)


def deserialize_commitment(data: bytes) -> dict:
    """Deserialize commitment from JSON bytes."""
