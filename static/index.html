...
<title>PSI-COMMIT — Prove You Said It First</title>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .sec-item::before { content: '✓'; color: var(--accent); font-family: var(--mono); flex-shrink: 0; }
  .sec-item.no::before { content: '✗'; color: #ff6060; }
...
<div class="modal-overlay" id="username-modal">
  <div class="modal">
    ...
    <button class="btn btn-primary" style="width:100%" onclick="submitUsername()" id="username-submit-btn">Set Username →</button>
  </div>
</div>
...
<div class="modal-overlay" id="comments-modal">
  <div class="modal" style="max-width:560px;max-height:80vh;overflow-y:auto">
    <button class="modal-close" onclick="document.getElementById('comments-modal').classList.remove('open')">✕</button>
    ...
    <div id="comments-input-area">
      ...
      <button class="btn btn-primary" onclick="submitComment()" style="font-size:0.72rem;padding:0.65rem 1.4rem">Post Comment →</button>
    </div>
...
<!-- NAV -->
<nav>
  <a class="nav-logo" href="#" onclick="showMainPage()">PSI — COMMIT</a>
  <ul class="nav-links">
...
<div id="profile-page">
  ...
  <div class="profile-tabs" style="display:flex;justify-content:space-between;align-items:center">
    <div style="display:flex">
      <button class="profile-tab active" onclick="switchProfileTab('public', this)">Public Commitments</button>
      <button class="profile-tab" onclick="switchProfileTab('private', this)">Private Commitments</button>
    </div>
...
<section id="hero">
  ...
  <p class="hero-sub">Commit to any prediction, hypothesis, or decision before the outcome — then reveal it later with mathematical proof you didn't cheat. No blockchain. No trust required.</p>
  <div class="hero-actions">
    <a href="#app" class="btn btn-primary">Make a Commitment →</a>
...
<section id="how">
  ...
  <div class="steps">
    <div class="step">
      ...
      <p>Write your prediction, hypothesis, or decision. A cryptographic digest is generated in your browser — your secret key never leaves your device.</p>
    </div>
    <div class="step">
      <div class="step-num">02</div>
      <div class="step-title">Wait</div>
      <p>Publish your commitment ID publicly. It's completely unreadable — nobody can guess your message from the hash, no matter how many tries.</p>
    </div>
    <div class="step">
      <div class="step-num">03</div>
      <div class="step-title">Reveal</div>
      <p>When you're ready, reveal your message and key. Anyone can verify — mathematically — that you wrote it before the outcome. The proof is permanent.</p>
    </div>
  </div>
</section>
...
<section id="usecases">
  ...
  <div class="cases-grid">
    ...
    <div class="case">
      <div class="case-icon">📈</div>
      <h3>Forecasters</h3>
      <p>Your Metaculus score is fine — but can you prove what you called to people who weren't on the platform? Build a portable, verifiable prediction record.</p>
    </div>
    <div class="case">
      <div class="case-icon">🏢</div>
...
<section id="app">
  ...
  <div class="tab-panel active" id="commit-panel">
    ...
    <div class="login-gate" id="commit-login-gate">
      ...
      <button class="btn btn-primary" onclick="openModal()">Sign in with Google →</button>
    </div>
    ...
    <div class="app-card" id="commit-form" style="display:none">
      ...
      <div class="field-group">
        ...
        <div class="visibility-toggle">
          <button class="vis-btn active" id="vis-public" onclick="setVisibility('public')">🌐 Public</button>
      ...
      <button class="btn btn-primary" onclick="createCommitment()">Generate Commitment →</button>
      ...
      <div class="result-box" id="commit-result">
        ...
        <div style="background:rgba(200,240,0,0.06);border:1px solid rgba(200,240,0,0.25);padding:1rem 1.25rem;margin-top:1.25rem;font-family:var(--mono);font-size:0.72rem;color:var(--accent);line-height:1.7">
          ⚠ SAVE THIS RECEIPT. Your secret key is not stored anywhere. If you lose it, you can never prove this commitment.
        </div>
        <div style="display:flex;gap:0.75rem;margin-top:1.25rem;flex-wrap:wrap">
          <button class="btn btn-primary" onclick="downloadPSC()" style="font-size:0.72rem;padding:0.7rem 1.4rem">↓ Download Files (.psc + .stamp.txt)</button>
          <button class="btn btn-outline" onclick="exportOTS()" style="font-size:0.72rem;padding:0.7rem 1.4rem" id="export-ots-btn">↓ Export .ots File</button>
  ...
  <div class="tab-panel" id="verify-panel">
    ...
    <div class="app-card" style="margin-bottom:1.5rem">
      ...
      <div class="field-group">
        <label class="field-label">Step 1 — Paste your commitment receipt (.psc) or text receipt</label>
      ...
      <div id="parsed-fields" style="display:none">
        <div style="font-family:var(--mono);font-size:0.68rem;letter-spacing:0.15em;color:var(--accent);text-transform:uppercase;margin-bottom:1rem;padding-bottom:0.5rem;border-bottom:1px solid var(--border)">Parsed from receipt ✓</div>
        <div class="field-group">
      ...
      <div class="field-group">
        <label class="field-label">Step 2 — Enter the original message to verify</label>
      ...
      <button class="btn btn-primary" onclick="verifyCommitment()">Verify Commitment →</button>
    ...
    <div class="app-card">
      ...
      <p style="font-size:0.85rem;color:#999;margin-bottom:1.5rem;line-height:1.7">Drop your <strong style="color:var(--white)">.ots</strong> and <strong style="color:var(--white)">.stamp.txt</strong> files to verify the Bitcoin timestamp independently — no trust in us required.</p>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.25rem">
        <div>
          <label class="field-label" style="margin-bottom:0.5rem;display:block">① Drop .ots file</label>
        ...
        <div>
          <label class="field-label" style="margin-bottom:0.5rem;display:block">② Drop .stamp.txt file</label>
      ...
      <button class="btn btn-primary" onclick="verifyOTS()" id="verify-ots-btn" disabled style="opacity:0.4">Verify Bitcoin Timestamp →</button>
      ...
      <div style="margin-top:1.5rem;font-family:var(--mono);font-size:0.65rem;color:#444;line-height:1.8">
        You can also verify on <a href="https://dgi.io/ots/" target="_blank" style="color:var(--muted)">dgi.io/ots</a> or <a href="https://opentimestamps.org" target="_blank" style="color:var(--muted)">opentimestamps.org</a> — drop .ots in box 1, .stamp.txt in box 2.
      </div>
    </div>
  </div>
...
<section id="security">
  ...
  <div style="max-width:820px;margin:3rem auto 0">
    <div style="border:1px solid var(--accent);padding:2rem;margin-bottom:2rem;position:relative">
      ...
      <p style="font-size:1.15rem;line-height:1.8;color:var(--white)">We cannot forge your commitments. We cannot change your timestamps. We cannot lie about when you committed — even if we wanted to. The math and Bitcoin make it impossible.</p>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:1px;background:var(--border);margin-bottom:2rem">
      <div style="background:var(--black);padding:1.75rem">
        ...
        <p style="font-size:0.82rem;color:#999;line-height:1.7">Your secret key is generated in your browser and never sent to our server. We store only the MAC — a mathematical fingerprint. Without your key, the message is permanently hidden.</p>
      </div>
      <div style="background:var(--black);padding:1.75rem">
      ...
      <div style="background:var(--black);padding:1.75rem">
        ...
        <p style="font-size:0.82rem;color:#999;line-height:1.7">Every commitment is anchored to the Bitcoin blockchain via OpenTimestamps. Nobody controls it — not us, not you, not anyone.</p>
      </div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1px;background:var(--border)">
      <div style="background:var(--black);padding:1.75rem">
        ...
        <ul style="font-size:0.82rem;color:#999;line-height:2.2;list-style:none">
          <li style="display:flex;gap:0.5rem"><span style="color:#ff4444">✗</span> Read your message before you reveal it</li>
          <li style="display:flex;gap:0.5rem"><span style="color:#ff4444">✗</span> Change what you committed to</li>
          <li style="display:flex;gap:0.5rem"><span style="color:#ff4444">✗</span> Backdate or alter your timestamp</li>
          <li style="display:flex;gap:0.5rem"><span style="color:#ff4444">✗</span> Forge a commitment on your behalf</li>
          <li style="display:flex;gap:0.5rem"><span style="color:#ff4444">✗</span> Prevent you from verifying independently</li>
        </ul>
      </div>
      <div style="background:var(--black);padding:1.75rem">
        ...
        <ul style="font-size:0.82rem;color:#999;line-height:2.2;list-style:none">
          <li style="display:flex;gap:0.5rem"><span style="color:var(--accent)">✓</span> Store your public MAC and timestamp</li>
          <li style="display:flex;gap:0.5rem"><span style="color:var(--accent)">✓</span> Submit your MAC to Bitcoin via OTS</li>
          <li style="display:flex;gap:0.5rem"><span style="color:var(--accent)">✓</span> Verify your key matches on reveal</li>
          <li style="display:flex;gap:0.5rem"><span style="color:var(--accent)">✓</span> Maintain the public wall</li>
          <li style="display:flex;gap:0.5rem"><span style="color:var(--accent)">✓</span> Keep the code open source forever</li>
        </ul>
      </div>
    </div>
  </div>
...
<section style="padding:7rem 2.5rem;border-top:1px solid var(--border)">
  ...
  <div class="security-grid">
    <div class="sec-col">
      ...
      <div class="sec-item">Cryptographic binding — you cannot change your message after committing</div>
      <div class="sec-item">Cryptographic hiding — the commitment reveals nothing until you reveal</div>
      <div class="sec-item">Bitcoin timestamp — every commitment is permanently anchored to the blockchain via OpenTimestamps</div>
      <div class="sec-item">Domain separation — prevents cross-context replay attacks</div>
      <div class="sec-item">Constant-time verification — immune to timing attacks</div>
      <div class="sec-item">Zero-knowledge server — your secret never leaves your browser</div>
      <div class="sec-item">HMAC-SHA256 — 256-bit security, same standard used by AWS and GitHub</div>
    </div>
    <div class="sec-col">
      <h3 class="not-provides">What this does NOT provide</h3>
      <div class="sec-item no">Anonymity — your username is attached to public commitments</div>
      <div class="sec-item no">Forward secrecy — if your key is compromised, that commitment can be forged</div>
      <div class="sec-item no">Legal enforceability — cryptographic proof is not the same as legal proof</div>
      <div class="sec-item no">Message recovery — if you lose your secret key, your commitment cannot be revealed</div>
    </div>
  </div>
  <div style="margin-top:3rem;padding:2rem;border:1px solid var(--border);max-width:680px">
    ...
    <p style="color:#999;font-size:0.9rem;line-height:1.7">The cryptographic core is MIT-licensed and publicly auditable. Don't trust us — read the code.</p>
    <a href="https://github.com/RayanOgh/psi-commit" target="_blank" class="btn btn-outline" style="margin-top:1.5rem;display:inline-flex">View Source on GitHub →</a>
  </div>
</section>
...
<section id="verify-guide" style="padding:7rem 2.5rem;border-top:1px solid var(--border)">
  ...
  <p style="max-width:680px;color:#999;font-size:0.95rem;line-height:1.8;margin:2rem 0 3rem">Your .ots file is a cryptographic proof anchored in Bitcoin. You can verify it independently on opentimestamps.org or using the open-source OTS client — no account needed, no trust in us required.</p>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;max-width:900px">
    <div style="border:1px solid var(--border);padding:2rem">
      ...
      <ol style="font-size:0.85rem;color:#999;line-height:2.2;padding-left:1.2rem">
        <li>Go to your profile and click <strong style="color:var(--white)">↓ .ots</strong> on any commitment</li>
        ...
        <li>It will show the Bitcoin block number and timestamp — independent of us</li>
      </ol>
    </div>
    <div style="border:1px solid var(--border);padding:2rem">
      ...
      <p style="font-size:0.78rem;color:#666;margin-top:1rem;line-height:1.6">The OTS client connects directly to Bitcoin nodes to verify the proof — completely independent of psicommit.com.</p>
    </div>
  </div>
  ...
  <div style="margin-top:2rem;padding:1.5rem 2rem;border:1px solid var(--border);max-width:900px;display:flex;gap:1rem;align-items:center;flex-wrap:wrap">
    <div style="font-family:var(--mono);font-size:0.72rem;color:var(--muted)">⚡ Note: Bitcoin confirmation takes 1-2 hours after your commitment is made. Until confirmed, the .ots file is a pending receipt. Once confirmed, it is permanently anchored in the Bitcoin blockchain — forever.</div>
  </div>
</section>

<!-- FOOTER -->
<footer>
  <div class="footer-logo">PSI — COMMIT</div>
...
async function ensureProfile(user) {
  ...
  // New user — show username picker modal
...
async function submitUsername() {
  ...
  if (taken) {
    document.getElementById('username-feedback').textContent = 'Username taken — try another.';
...
async function createCommitment() {
  ...
  const card = [
    ...
    `NONCE     ${nonce}`,
    `${'─'.repeat(48)}`,
    `SECRET KEY (keep private — do not share until reveal)`,
    `${key}`,
    `${'─'.repeat(48)}`,
...
function buildPSCData(commitment) {
  // Full receipt including secret key and message — for user download only, never sent to server
  return {
    version: '1',
    id: commitment.id,
    mac: commitment.mac,
...
async function buildStampDigest(commitment) {
  // SHA256 of the stamp file text — this is what gets submitted to OTS
...
function promptAndDownloadPSC() {
  // Called automatically after commit — downloads both files
...
function exportOTS(commitment = null) {
  ...
  if (!otsHex) {
    alert('OTS receipt not yet available. Bitcoin anchoring is in progress — try again in a few minutes, or re-download from your profile once anchoring completes.');
    return;
  }
...
async function downloadCommitmentPSC(commitmentId) {
  ...
  // Note: key is NOT stored server-side — we can only include what we have
  const data = {
    version: '1',
    id: c.id,
    message: c.revealed_message || null,
    mac: c.mac,
    key: null, // not stored server-side — user must have saved their original .psc
    nonce: c.nonce,
    domain: c.domain,
    context: c.context,
    ...
    note: 'Secret key not included — retrieve from your original .psc receipt.',
    created_by: 'psicommit.com'
  };
...
async function verifyCommitment() {
  ...
  try {
    ...
    msgEl.textContent = valid
      ? 'VERIFIED — This commitment matches the message and key.'
      : 'FAILED — The commitment does not match. The message or key may have been altered.';
  } catch(e) {
    resultEl.className = 'verify-result show invalid';
    iconEl.textContent = '✗';
...
function buildWallItem(item, isOwner = false) {
  ...
  const msg = revealed
    ...
    : '<span style="color:var(--muted);font-style:italic;font-size:0.85rem">Sealed — message hidden until reveal</span>';
  ...
  if (item.ots_status === 'confirmed' && item.bitcoin_block) {
    otsLine = `<div style="font-family:var(--mono);font-size:0.65rem;color:var(--accent2);margin-top:0.3rem">
      ⛓ Bitcoin block #${item.bitcoin_block}
      <a href="https://blockstream.info/block-height/${item.bitcoin_block}" target="_blank" style="color:var(--accent2);margin-left:0.4rem;opacity:0.7">verify ↗</a>
    </div>`;
  } else if (item.ots_status === 'submitted') {
    otsLine = `<div style="font-family:var(--mono);font-size:0.65rem;color:#888;margin-top:0.3rem">⏳ Bitcoin anchoring in progress...</div>`;
  }
  ...
  <div class="wall-item">
    <div>
      ...
      <div style="display:flex;gap:0.5rem;margin-top:0.75rem;flex-wrap:wrap">
        ...
        ${revealed ? `<button class="btn btn-outline" style="font-size:0.65rem;padding:0.4rem 0.9rem;color:var(--accent2);border-color:var(--accent2)" onclick="openComments('${item.id}')">💬 Comments</button>` : ''}
        ${isOwner ? `<button class="btn btn-outline" style="font-size:0.65rem;padding:0.4rem 0.9rem;color:var(--accent2)" onclick="downloadCommitmentPSC('${item.id}')">↓ .psc</button>` : ''}
        ${isOwner ? `<button class="btn btn-outline" style="font-size:0.65rem;padding:0.4rem 0.9rem;color:var(--accent2)" onclick="downloadCommitmentStamp('${item.id}', '${item.mac}', '${item.committed_at || item.timestamp}')">↓ .stamp.txt</button>` : ''}
        ${isOwner ? `<button class="btn btn-outline" style="font-size:0.65rem;padding:0.4rem 0.9rem;color:var(--accent2)" onclick="exportCommitmentOTS('${item.id}')">↓ .ots</button>` : ''}
...
function validateEditUsername() {
  ...
  else { feedback.textContent = '✓ Looks good'; feedback.style.color = 'var(--accent)'; }
}
...
function handleOTSFile(file) {
  ...
  reader.onload = (e) => {
    ...
    document.getElementById('ots-drop-label').textContent = `✓ ${file.name} (${otsFileBytes.length} bytes)`;
    checkOTSReady();
  };
...
function handleStampFile(file) {
  ...
  reader.onload = (e) => {
    ...
    zone.classList.add('loaded');
    document.getElementById('stamp-drop-label').textContent = `✓ ${file.name}`;
    checkOTSReady();
  };
    ...
    if (data.status === 'confirmed') {
      ...
      msgEl.innerHTML = `Bitcoin Timestamp Verified<br>
        <span style="font-size:0.75rem;color:#999;font-family:var(--mono)">
          Commitment ID: ${escapeHtml(stampData['id'] || '—')}<br>
          Bitcoin block: #${data.bitcoin_block}<br>
          Stamp digest: ${stampHex.slice(0, 16)}...<br>
          <a href="https://blockstream.info/block-height/${data.bitcoin_block}" target="_blank" style="color:var(--accent2)">View block on blockstream.info ↗</a>
        </span>`;
      ...
      resultEl.className = 'verify-result show invalid';
      iconEl.textContent = '✗';
      msgEl.textContent = data.message || 'Verification failed — the .ots and .stamp.txt files do not match.';
    }
  } catch(e) {
    resultEl.className = 'verify-result show invalid';
    iconEl.textContent = '✗';
